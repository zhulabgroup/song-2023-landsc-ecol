...
combine_df_ori_fore<-combine_df_ori %>% filter(year >midyear)
  ...
  p<-
    ...
    geom_point(aes(x=date, y=pheno, col="observed phenology"),alpha=0.5)+
    geom_point(aes(x=date, y=value, col="predicted phenology"))+
  ...
  ggplot(combine_df_ori)+
    geom_point(aes(x=pheno, y=value, col=period),alpha=0.5)+
    # geom_smooth(aes(x=pheno, y=value),alpha=0.5, method="lm")+
    geom_abline(intercept = 0, slope=1)+
    ggpubr::stat_cor(
      aes(
        x = pheno, y = value, group = period,
