...
path_results<-paste0(path_sub, "results")
dir.create(path_results, recursive = T)

########################
# Get new data
########################
X_all<-read_csv(paste0(path_train, "/X.csv"), col_types = rep("d", ndim) %>% paste(collapse =""))%>% as.matrix()
Y_all<-read_csv(paste0(path_train, "/Y.csv"), col_types="n" )%>% as.matrix()
D_all<-read_csv(paste0(path_train, "/D.csv") , col_types="D" ) %>% as.matrix()
P_all<-read_csv(paste0(path_train, "/P.csv") , col_types="n") %>% as.matrix()
...
log_p_valid_new <-
  foreach(
    ...
    for (k in 1:num_sample) {
      ...
      res <- GPSDM(pars = pars_new[, i, drop = F], distMat = distMat, basisX = X_basis, basisP = P_basis,basisD=D_basis, basisY = Y_basis, newX = X_valid[minibatch, , drop = F], newP = P_valid[minibatch, , drop = F],  newD = D_train[minibatch, , drop = F], newY = Y_valid[minibatch, , drop = F],priors=priors, mode = c("optimize"))
      loglik_res[k] <- -res$neglpost
      print(k)
    }
