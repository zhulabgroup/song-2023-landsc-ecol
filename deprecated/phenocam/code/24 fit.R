...
Y_fit<-Var_fit<-matrix(NA, nrow=nrow(P_fit), ncol = steps,
                       dimnames = list(as.character(1:nrow(coord_df)),
                                       as.character(date_list[(fit_start + 1):(fit_start + steps)] )) 
)
for (g in 1:length(step_groups)) {
  ...
  if (nrow(newX)>0) {
    ...
    for(s in 1:nrow(P_fit)) {
      ...
      #store prediction
      Y_fit[P_fit[s,1],newD[newP==s]]<-res_df$weighted_mean
      Var_fit[P_fit[s,1],newD[newP==s]]<-res_df$weighted_variance
      
      xnew[P_fit[s,1],newD[newP==s],1]<-res_df$weighted_mean
      Sigmanew[P_fit[s,1],newD[newP==s],1]<-res_df$weighted_variance
    }
  }
...
climatology_df<-ts %>% 
  ...
  combine_df_ori<-ts %>% 
    ...
    left_join(data.frame(site=ts %>% pull(site) %>% unique()) %>% mutate(siteid=row_number()),by="site") %>% 
    dplyr::select(-site) %>% 
    rename(site=siteid) %>% 
