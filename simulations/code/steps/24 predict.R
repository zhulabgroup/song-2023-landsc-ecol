path_prediction <- paste0(path_sub, "prediction/")
dir.create(path_prediction, recursive = T)

# define scope of prediction
...
# make predictions
step_groups <- split(seq(1:steps), ceiling(seq(1:steps) / 100)) # predict in chunks
...
for (g in 1:length(step_groups)) {
  ...
  if (nrow(newX) > 0) {
    ...
    for (s in 1:nrow(P_fit)) {
      ...
      res_df <- bind_rows(
        as.data.frame(means) %>% mutate(stat = "means") %>% mutate(t = row_number()),
        ...
        group_by(t) %>%
        summarize(
          weighted_mean = weighted.mean(means, w = exp(log_p_valid - mean(log_p_valid))),
...
# organize predictions into data frames
...
# save predictions
write_csv(fit_df, paste0(path_prediction, "fit.csv"))
write_csv(var_fit_df, paste0(path_prediction, "var_fit.csv"))

# process prediction data
fit_df <- left_join(fit_df, var_fit_df, by = c("site", "date")) %>%
  dplyr::rename(value = value.x, variance = value.y) %>%
  mutate(
    lower = value - 1.96 * sqrt(variance),
...
fit_df_ori <- fit_df %>%
  ...
  mutate(
    value = (value + 0.5) * range + lower.scale,
...
combine_df_ori <- ts %>%
  ...
  mutate(
    mismatch_actual = pheno - pheno_mis,
...
write_csv(combine_df_ori, paste0(path_output, param, ".csv"))
